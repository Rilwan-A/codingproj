...
def config_trainer_param_adjust(config_trainer, batched_dset_len=None ):
    ...
    if not (config_trainer.steps_per_epoch).is_integer():
...
class ImputationTrainer(tf.keras.Model):
    ...
    def train(config_trainer, config_data, config_diffusion, config_model ):
        ...
        prog_bar = tfa.callbacks.TQDMProgressBar()
        ...
        #Optimizer Setup        
        if config_trainer.optimizer == 'adam':
            boundaries = [config_trainer.steps_per_epoch//4, config_trainer.steps_per_epoch, 3*config_trainer.steps_per_epoch]
            values = [ 1*config_trainer.learning_rate, 1.5*config_trainer.learning_rate, 1.25*config_trainer.learning_rate , 1.0*config_trainer.learning_rate]
            ...
            boundaries = [config_trainer.steps_per_epoch//10, config_trainer.steps_per_epoch, 5*config_trainer.steps_per_epoch, 10*config_trainer.steps_per_epoch ]
            values = [ 1*config_trainer.learning_rate, 2*config_trainer.learning_rate, 2*config_trainer.learning_rate , 1.5*config_trainer.learning_rate, config_trainer.learning_rate]

            schedule = tf.keras.optimizers.schedules.PiecewiseConstantDecay(boundaries, values)    
            
            schedule = WarmUp( config_trainer.learning_rate, schedule, config_trainer.steps_per_epoch) # config_trainer.steps_per_epoch//10 )
                        
            optimizer = ks.optimizers.experimental.Adafactor(
                                                            schedule,
                                                            #  beta_2_decay=-0.8,
                                                            #  clip_threshold=1.0,
                                                            #  relative_step=False,
                                                            ...
                                                            #  use_ema=True,
                                                             #  ema_momentum=0.99,
                                                            #  ema_overwrite_frequency=config_trainer.steps_per_epoch
                                                             )
